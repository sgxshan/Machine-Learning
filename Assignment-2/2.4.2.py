...
def TrainNeuralNetwork(batch, learningRate,decay,num_hid_unit,dropout):
    ...
    for i in xrange(0,epochs):
        ...
        for step in xrange(0,updateNUm):
            train_, trainerr, trainyhat, w= sess.run([train, totalLoss, y_predicted,W_hid_layer], feed_dict={X: traindata[step], y_target: traintargets[step]} )
            if (dropout==0.5 and i==2):
                w = np.transpose(w)
                w1 = np.reshape(w,[1000,28,28])
                for i in xrange(1, 1001):
                    fig = plt.subplot(32, 32, i)
                    plt.imshow(w1[i - 1], cmap=plt.cm.gray)
                    plt.axis('off')
                plt.show()
                print 'finish'
...
epochs =15
decay = 3*math.exp(-4)
TrainNeuralNetwork(500,0.001,decay,1000,0.5)

...
def log_prob_density(X,U,var,pi,D):
    distance = pairwise_distance(X,U)
    log_likelihood = -D / 2 * tf.log(2. * math.pi * tf.square(var)) - tf.div(distance, (2. * tf.square(var)))
    log_prior = logsoftmax(pi)
    log_magrinal = reduce_logsumexp(log_prior + log_likelihood, 1, True)

...
def runGraph(learning_rate,K,D):
    ...
    for step in range(0,250):
        u,train_process= sess.run([U, train], feed_dict={X:train_data})
    u_trained = u
    X = tf.placeholder("float")
    Y = tf.placeholder("float")
    distance = pairwise_distance(X,Y)
    index = tf.argmin(distance,1)
    cluster = tf.gather(Y,index)
    square_loss = tf.reduce_sum(tf.reduce_sum(tf.square(X - cluster),1),0)
    loss = sess.run(square_loss, feed_dict={X:valid_data, Y:u_trained})
    return u_trained
...
# for k in range(1,50):
#     loss = runGraph(0.001,k)
#     print ('k = %s,'%k + 'loss of validation set is: %s'%loss)
